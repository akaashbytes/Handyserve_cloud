...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // ── Steps 7–9: Verify user still exists and is active ─────────────
        if (email != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            try {
                ...
                log.warn("[JWT-FILTER] Token valid but user not found: {}", email);
                writeError(response, request, HttpStatus.UNAUTHORIZED,
                        "User account no longer exists. Please register again.");
                return;
            }

            // UserDetailsServiceImpl already throws UsernameNotFoundException
            // for blocked users, so if we get here the user is active.
            ...
            UsernamePasswordAuthenticationToken auth =
                    new UsernamePasswordAuthenticationToken(
                            ...
                            userDetails.getAuthorities()    // ROLE_ADMIN / ROLE_PROVIDER / ROLE_CUSTOMER
                    );
            auth.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
            SecurityContextHolder.getContext().setAuthentication(auth);

            log.debug("[JWT-FILTER] Authenticated user: {} | authorities: {}", email, userDetails.getAuthorities());
        }
